...
def train():
    ...
    net = QuantWeightActBiasLeNet(weight_bit_width, bit_width)
            ...
            if accuracy > max_acc:
                try:
                    export_onnx_qcdq(net, torch.randn(36, 3, 32, 32),
                                     export_path=f'./onnx/model3_{weight_bit_width}_{bit_width}.onnx')
                    print(f"save model   ./onnx/model3_{weight_bit_width}_{bit_width}.onnx     successfully")
    ...
    plt.legend()
    plt.savefig(f"./statistics/model3_loss_{weight_bit_width}_{bit_width}")
    plt.close()

    plt.xlabel("epoch")
    plt.plot(Acc, label="accuracy")
    plt.legend()
    plt.savefig(f"./statistics/model3_accuracy_{weight_bit_width}_{bit_width}")
    plt.close()
